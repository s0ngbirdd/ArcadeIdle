                ...
                for (int z = 0; z < _pile.GetLength(1); z++)
                {
                    for (int y = 0; y < _pile.GetLength(2); y++)
                    {
                        if (_pile[x, z, y] == null && _canSpawn)
                        {
                            ...
                            GameObject sword = ObjectPool.ObjectPool.Instance.GetSwordPooledObject();
                            sword.transform.position = transform.position;
                            sword.transform.rotation = Quaternion.identity;
                            sword.transform.SetParent(transform);
                            sword.SetActive(true);

                            _spawnTween = sword.transform.DOLocalJump(new Vector3(x - _spawnOffsetX, y + _spawnOffsetY, z + _spawnOffsetZ), 2, 1, _animationDuration).SetEase(Ease.Linear).OnComplete(() =>
                            {
