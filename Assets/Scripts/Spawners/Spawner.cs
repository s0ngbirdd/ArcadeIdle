                ...
                for (int y = _pile.GetLength(2) - 1; y > -1; y--)
                {
                    if (_pile[x, z, y] != null && _canDespawn)
                    {
                        ...
                        if (backpackPosition != Vector3.zero)
                        {
                            ...
                            _despawnTween = _pile[x, z, y].transform.DOLocalJump(backpackPosition, 2, 1, _spawnDuration).SetEase(Ease.Linear).OnComplete(() =>
                            {
                                _canDespawn = true;
                                _despawnTween.Kill();
                            });
                            
                            _rotateTween = _pile[x, z, y].transform.DOLocalRotate(Vector3.zero, _spawnDuration).SetEase(Ease.Linear).OnComplete(() =>
                            {
                                _rotateTween.Kill();
                            });
