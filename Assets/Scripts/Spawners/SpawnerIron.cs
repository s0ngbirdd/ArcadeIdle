                ...
                for (int z = 0; z < _pile.GetLength(1); z++)
                {
                    for (int y = 0; y < _pile.GetLength(2); y++)
                    {
                        if (_pile[x, z, y] == null && _canSpawn)
                        {
                            ...
                            GameObject iron = ObjectPool.ObjectPool.Instance.GetIronPooledObject();
                            iron.transform.position = transform.position;
                            iron.transform.rotation = Quaternion.identity;
                            iron.transform.SetParent(transform);
                            iron.SetActive(true);

                            _spawnTween = iron.transform.DOLocalJump(new Vector3(x - _spawnOffsetX, y + _spawnOffsetY, z + _spawnOffsetZ), 2, 1, _animationDuration).SetEase(Ease.Linear).OnComplete(() =>
                            {
